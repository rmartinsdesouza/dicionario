<?php
 ...
 // 	require_once("Rest.inc.php");
	...
	// 		if($id > 0){	
	// 			$query="SELECT distinct (ID), NOME FROM pessoa where ID=$id";
	// 			$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
	// 			if($r->num_rows > 0) {
	// 				$result = $r->fetch_assoc();	
	// 				$this->response($this->json($result), 200); // send user details
	...
	// 	}
	// 	private function updatePessoa(){
	// 		if($this->get_request_method() != "POST"){
	// 			$this->response('',406);
	// 		}
	// 		$pessoa = json_decode(file_get_contents("php://input"),true);
	// 		$id = (int)$pessoa['ID'];
	// 		$column_names = array('NOME');
	// 		$keys = array_keys($pessoa['pessoa']);
	// 		$columns = '';
	// 		$values = '';
	// 		foreach($column_names as $desired_key){ // Check the pessoa received. If key does not exist, insert blank into the array.
	// 		   if(!in_array($desired_key, $keys)) {
	// 		   		$$desired_key = '';
	// 			}else{
	// 				$$desired_key = $pessoa['pessoa'][$desired_key];
	// 			}
	// 			$columns = $columns.$desired_key."='".$$desired_key."',";
	// 		}
	// 		$query = "UPDATE pessoa SET ".trim($columns,',')." WHERE ID=$id";
	// 		if(!empty($pessoa)){
	// 			$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
	// 			$success = array('status' => "Success", "msg" => "Pessoa ".$id." modifição concluída.", "data" => $pessoa);
	// 			$this->response($this->json($success),200);
	// 		}else
	// 			$this->response('',204);	// "No Content" status
	// 	}
		
	// 	private function deletePessoa(){
	...
	// 			$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
	// 			$success = array('status' => "Success", "msg" => "Um registro deletado com sucesso.");
	// 			$this->response($this->json($success),200);
	// 		}else
	// 			$this->response('',204);	// If no records "No Content" status
	// 	}
